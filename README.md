...
CommandCode, Codex, Claude, Cursor의 사용량을 작은 Windows 창에서 함께 확인하는 모니터링 도구야.
...
- Python 3.10 이상
- Codex CLI 설치 및 로그인
- Claude 열을 쓸 경우 Claude Code 설치 및 로그인
...
Claude 사용량도 별도 토큰을 저장하지 않는다. 이 컴퓨터에서 Claude Code에 로그인해 두면 `~/.claude/.credentials.json`의 OAuth 토큰을 요청 순간에만 읽어 `https://api.anthropic.com/api/oauth/usage`에서 5시간 / 7일 소진율을 가져온다. `CLAUDE_CONFIG_DIR`을 설정했다면 그 폴더를 먼저 본다.

Claude 액세스 토큰은 8시간이면 만료되기 때문에, 하룻밤 자고 아침에 처음 실행하면 이미 만료돼 있다. 이때는 같은 파일에 들어 있는 리프레시 토큰으로 직접 갱신하고 이어서 사용량을 읽는다. `claude`를 먼저 실행할 필요가 없다. 갱신에는 다음 안전장치가 걸려 있다.

- 액세스 토큰이 실제로 만료됐을 때만 갱신한다. 미리 갱신하지 않는다.
- 덮어쓰기 전에 `~/.claude/.credentials.json.ccusage.bak`으로 백업하고, 임시 파일에 쓴 뒤 원자적으로 교체하고, 다시 읽어 검증한다. 어느 단계든 실패하면 원본으로 되돌린다. 갱신 대상 외의 필드는 그대로 둔다.
- 락 파일로 한 번에 하나의 갱신만 수행한다. 락을 잡은 뒤 파일을 다시 읽어, 그 사이 Claude Code가 토큰을 갱신했으면 요청을 보내지 않고 그 토큰을 쓴다.
- 갱신이 거절되면 자격증명 파일이 다시 바뀔 때까지 재시도하지 않는다. 일시적 실패는 5분 쉬었다 다시 시도한다.
- 이 동작이 마음에 들지 않으면 `config.json`에 `"claude": { "auto_refresh": false }`를 넣는다. 그러면 파일을 전혀 쓰지 않고, 만료 시 `claude`를 실행하라는 오류만 표시한다.
...
- `claude_usage.py`: 로컬 Claude Code 세션으로 Claude 구독의 5시간 / 7일 사용량을 읽는 로직
...
Claude 사용량은 Claude Code가 `/usage`에 쓰는 것과 같은 `/api/oauth/usage` 인터페이스에 의존한다. 응답 구조가 바뀌면 Claude 표시 기능을 조정해야 할 수 있다.
...
- Claude 인증은 설치된 Claude Code의 로그인 상태를 사용하며, OAuth 토큰은 저장하거나 로그에 남기지 않는다.
